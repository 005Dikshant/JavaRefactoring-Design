...
public abstract class BaseParamFilterReader
	...
	//public abstract boolean match(); // this is template design method
	...
	protected int readExtracted(Consumer<Parameter> initializer, Supplier<Boolean> matcher) throws IOException {
		if (!getInitialized()) {
			Parameter[] params = getParameters();
			if (params != null) {
				for (int i = 0; i < params.length; i++) {
					Parameter parameter = params[i];
					//intializeParameter(parameter);
	...
	//public abstract void initializeParameters(Parameter parameter);
